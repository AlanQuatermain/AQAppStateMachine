...
- (void) testAllKeys {
	...
	STAssertEqualObjects(
		[srd allKeys],
		[[nsd allKeys] sortedArrayUsingSelector: @selector(compare:)],
		@"arrays not equal");
}
...
- (void) testAllKeysForObject {
	...
	for (int i = 0; i <= 9; ++i) {
		...
		NSArray *a2 = [[nsd allKeysForObject: value] sortedArrayUsingSelector: @selector(compare:)];
...
- (void) testAllValues {
	...
	NSArray *a1 = [[srd allValues] sortedArrayUsingComparator:^NSComparisonResult(__strong id obj1, __strong id obj2) { return ( -([obj1 compare: obj2]) ); }];
	NSArray *a2 = [[nsd allValues] sortedArrayUsingComparator:^NSComparisonResult(__strong id obj1, __strong id obj2) { return ( -([obj1 compare: obj2]) ); }];
...
- (void) testGetObjectsAndKeys {
	...
	id __unsafe_unretained keys[1001];
	id __unsafe_unretained values[1001];
	...
	for (int i = 1; i < 1000; i += 19) {
		...
		STAssertTrue(
			[keys[i] compare: keys[i - 1]] == NSOrderedDescending,
...
- (void) testKeysSortedByValueUsingSelector {
	...
	for (int i = 1; i < 1000; ++i) {
		...
		STAssertTrue(
			[prevValue compare: value] <= NSOrderedSame,
...
- (void) testEntryEnumerator {
	...
	for (NSObject<SortedDictionaryEntry> *entry in [srd entryEnumerator]) {
		...
		if (prevKey) {
			STAssertTrue(
				[prevKey compare: [entry key]] == NSOrderedAscending,
...
- (void) testKeyEnumerator {
	...
	for (id key in [srd keyEnumerator]) {
		...
		if (prevKey) {
			STAssertTrue(
				[prevKey compare: key] == NSOrderedAscending,
...
- (void) testObjectEnumerator {
	...
	NSArray			*sortedKeys	= [[nsd allKeys] sortedArrayUsingSelector: @selector(compare:)];
	NSMutableArray	*values		= [NSMutableArray arrayWithCapacity: 1000];
	for (int i = 0; i < 1000; ++i) {
...
- (void) testBlockEnumerator {
	...
	// iterate using the object enumerator, check that values match expectations
	__block int index = 0;
	[srd enumerateKeysAndObjectsUsingBlock: ^(__strong id key, __strong id obj, BOOL *stop) {
		id expectedValue = [values objectAtIndex: index];
		id expectedKey   = [sortedKeys objectAtIndex: index];
		STAssertEqualObjects(key, expectedKey, @"Expected equal keys at index %d", index);
		STAssertEqualObjects(obj, expectedValue, @"Expected equal objects at index %d", index);
		++index;
	}];
...
- (void) testReverseEntryEnumerator {
	...
	for (NSObject<SortedDictionaryEntry> *entry in [srd reverseEntryEnumerator]) {
		...
		if (prevKey) {
			STAssertTrue(
				[[entry key] compare: prevKey] == NSOrderedAscending,
...
- (void) testReverseKeyEnumerator {
	...
	for (id key in [srd reverseKeyEnumerator]) {
		...
		if (prevKey) {
			STAssertTrue(
				[key compare: prevKey] == NSOrderedAscending,
...
- (void) testReverseBlockEnumerator {
	// prepare an array of all values in the reference NSDictionary, sorted by keys
	NSArray			*sortedKeys	= [[nsd allKeys] sortedArrayUsingComparator:^NSComparisonResult(__strong id obj1, __strong id obj2) { return ( -([obj1 compare: obj2]) ); }];
	NSMutableArray	*values		= [NSMutableArray arrayWithCapacity: 1000];
	for (int i = 0; i < 1000; ++i) {
		NSString *key = [sortedKeys objectAtIndex: i];
		NSString *value = [key substringFromIndex: [key length] - 1];
	...
	// iterate using the object enumerator, check that values match expectations
	__block int index = 0;
	[srd enumerateKeysAndObjectsWithOptions: NSEnumerationReverse usingBlock: ^(__strong id key, __strong id obj, BOOL *stop) {
		id expectedValue = [values objectAtIndex: index];
		id expectedKey   = [sortedKeys objectAtIndex: index];
		STAssertEqualObjects(key, expectedKey, @"Expected equal keys at index %d", index);
		STAssertEqualObjects(obj, expectedValue, @"Expected equal objects at index %d", index);
