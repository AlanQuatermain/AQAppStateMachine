...
- (void) checkDict: (id) d {
	...
	for (int i = 1; i < 1000; ++i) {
		...
		STAssertTrue([prevKey isLessThan: key], @"Expected keys in ascending order, but found %@ before %@", prevKey, key);
...
- (void) testSetObjectForKey {
	...
	MutableSortedDictionary *d = [[[MutableSortedDictionary alloc] init] autorelease];
	for (int i = 0; i < 200; ++i) {
...
- (void) testAddEntriesFromDictionary {
	...
	NSMutableDictionary *d2 = [[[NSMutableDictionary alloc] init] autorelease];	
	for (int i = 0; i < 1000; ++i) {
		NSString *key	= [NSString stringWithFormat: @"%d", i];
		NSString *value = [NSString stringWithFormat: @"%d", i % 10];
...
- (void) testAddEntriesFromSortedDictionary {
	...
	// create a secondary dictionary, fill it with the test values
	MutableSortedDictionary *d2 = [[[MutableSortedDictionary alloc] init] autorelease];	
	for (int i = 0; i < 1000; ++i) {
		NSString *key	= [NSString stringWithFormat: @"%d", i];
		NSString *value = [NSString stringWithFormat: @"%d", i % 10];
		[d2 setObject: value forKey: key];
