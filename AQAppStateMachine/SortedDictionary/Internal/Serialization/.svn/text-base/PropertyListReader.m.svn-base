...
@implementation Context
	...
	- (id) initWithContainer: (id) aContainer andState: (State) aState {
		if (self = [super init]) { container = [aContainer retain]; state = aState; key = nil; } return self;
	}
...
- (void)		parser: (NSXMLParser *) aParser
	...
	if ([elementName isEqualToString: @"dict"]) {
		switch (state) { case waitingForKey: case done: plist = nil; [aParser abortParsing]; return; }
...
- (void)		parser: (NSXMLParser *) aParser 
	...
	switch ([context state]) {
		case waitingForPlist:	// reached final </plist> tag
			plist = [readObject retain];			// we'll return this from the read method
